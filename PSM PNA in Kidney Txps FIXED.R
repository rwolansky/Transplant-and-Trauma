# Simplified PSM Analysis for Kidney Transplant - Binary Outcome (Pneumonia)
library(MatchIt)
library(tableone)
library(cobalt)
library(ggplot2)
...
print(table(my_data$pna, my_data$kidneytxp))
...
pre_outcome_table <- table(my_data$pna, my_data$kidneytxp, 
                           dnn = c("Pneumonia", "Kidney Transplant"))
print(pre_outcome_table)

# Calculate pre-matching pneumonia rates
pre_rates <- prop.table(pre_outcome_table, margin = 2)
pre_control_rate <- pre_rates[2, 1]
pre_treated_rate <- pre_rates[2, 2]

cat(sprintf("\nPre-Matching Pneumonia Rates:\n"))
...
love_plot <- love.plot(psmatch,
                       ...
                       position = "bottom",
                       var.names = var.names,
                       title = "Balance Before and After Matching: Kidney Transplant Pneumonia Analysis") +
  theme_bw() +
  theme(
...
treated_outcomes <- matched_pairs$pna[matched_pairs$kidneytxp == 1]
control_outcomes <- matched_pairs$pna[matched_pairs$kidneytxp == 0]
...
concordant_both_no_pneumonia <- mcnemar_table[1, 1]  # Both no pneumonia
concordant_both_pneumonia <- mcnemar_table[2, 2]  # Both have pneumonia
discordant_control_pneumonia <- mcnemar_table[2, 1]  # Control has pneumonia, treated does not
discordant_treated_pneumonia <- mcnemar_table[1, 2]  # Control no pneumonia, treated has pneumonia

total_pairs <- sum(mcnemar_table)

cat(sprintf("\nPaired Outcomes Summary:\n"))
cat(sprintf("Total matched pairs: %d\n", total_pairs))
cat(sprintf("Both had no pneumonia: %d pairs (%.1f%%)\n", 
            concordant_both_no_pneumonia, (concordant_both_no_pneumonia/total_pairs)*100))
cat(sprintf("Both had pneumonia: %d pairs (%.1f%%)\n", 
            concordant_both_pneumonia, (concordant_both_pneumonia/total_pairs)*100))
cat(sprintf("Control had pneumonia, Treated did not: %d pairs (%.1f%%)\n", 
            discordant_control_pneumonia, (discordant_control_pneumonia/total_pairs)*100))
cat(sprintf("Control had no pneumonia, Treated did: %d pairs (%.1f%%)\n", 
            discordant_treated_pneumonia, (discordant_treated_pneumonia/total_pairs)*100))

# Perform McNemar's test
mcnemar_result <- mcnemar.test(mcnemar_table, correct = TRUE)

# Calculate effect sizes for matched pairs
treated_pneumonia_rate <- sum(treated_outcomes) / length(treated_outcomes)
control_pneumonia_rate <- sum(control_outcomes) / length(control_outcomes)
matched_risk_diff <- treated_pneumonia_rate - control_pneumonia_rate

# Calculate odds ratio for McNemar's test
# OR = (discordant pairs in favor of treatment) / (discordant pairs against treatment)
if(discordant_treated_pneumonia > 0) {
  mcnemar_or <- discordant_treated_pneumonia / discordant_control_pneumonia
} else {
  mcnemar_or <- Inf
}

# Calculate confidence interval for OR
if(discordant_control_pneumonia > 0 && discordant_treated_pneumonia > 0) {
  log_or <- log(mcnemar_or)
  se_log_or <- sqrt(1/discordant_control_pneumonia + 1/discordant_treated_pneumonia)
...
clogit_model <- clogit(pna ~ kidneytxp + strata(subclass), data = matched_data)
...
cat(sprintf("Discordant pairs: %d (analysis based on these pairs)\n", 
            discordant_control_pneumonia + discordant_treated_pneumonia))

cat(sprintf("\nPost-Matching Effect Sizes:\n"))
cat(sprintf("Risk Difference: %.3f (%.1f percentage points)\n", matched_risk_diff, matched_risk_diff * 100))
cat(sprintf("Control group pneumonia rate: %.1f%% (%d/%d)\n", 
            control_pneumonia_rate * 100, sum(control_outcomes), length(control_outcomes)))
cat(sprintf("Treated group pneumonia rate: %.1f%% (%d/%d)\n", 
            treated_pneumonia_rate * 100, sum(treated_outcomes), length(treated_outcomes)))
...
if(mcnemar_result$p.value < 0.05) {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows statistically significant difference in pneumonia risk between matched pairs ***\n")
} else {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows no statistically significant difference in pneumonia risk between matched pairs ***\n")
}
