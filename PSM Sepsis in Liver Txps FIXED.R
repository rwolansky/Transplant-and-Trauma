# Simplified PSM Analysis for Liver Transplant - Binary Outcome (Sepsis)
library(MatchIt)
library(tableone)
library(cobalt)
library(ggplot2)
...
print(table(my_data$sepsis, my_data$livertxp))
...
love_plot <- love.plot(psmatch,
                       ...
                       position = "bottom",
                       var.names = var.names,
                       title = "Balance Before and After Matching: Liver Transplant Sepsis Analysis") +
  theme_bw() +
  theme(
...
matched_pairs <- matched_data[order(matched_data$subclass, matched_data$livertxp), ]

# Create data frame with pairs side by side
treated_outcomes <- matched_pairs$sepsis[matched_pairs$livertxp == 1]
control_outcomes <- matched_pairs$sepsis[matched_pairs$livertxp == 0]
...
concordant_both_no_sepsis <- mcnemar_table[1, 1]  # Both no sepsis
concordant_both_sepsis <- mcnemar_table[2, 2]  # Both have sepsis
discordant_control_sepsis <- mcnemar_table[2, 1]  # Control has sepsis, treated does not
discordant_treated_sepsis <- mcnemar_table[1, 2]  # Control no sepsis, treated has sepsis

total_pairs <- sum(mcnemar_table)

cat(sprintf("\nPaired Outcomes Summary:\n"))
cat(sprintf("Total matched pairs: %d\n", total_pairs))
cat(sprintf("Both had no sepsis: %d pairs (%.1f%%)\n", 
            concordant_both_no_sepsis, (concordant_both_no_sepsis/total_pairs)*100))
cat(sprintf("Both had sepsis: %d pairs (%.1f%%)\n", 
            concordant_both_sepsis, (concordant_both_sepsis/total_pairs)*100))
cat(sprintf("Control had sepsis, Treated did not: %d pairs (%.1f%%)\n", 
            discordant_control_sepsis, (discordant_control_sepsis/total_pairs)*100))
cat(sprintf("Control had no sepsis, Treated did: %d pairs (%.1f%%)\n", 
            discordant_treated_sepsis, (discordant_treated_sepsis/total_pairs)*100))

# Perform McNemar's test
mcnemar_result <- mcnemar.test(mcnemar_table, correct = TRUE)

# Calculate effect sizes for matched pairs
treated_sepsis_rate <- sum(treated_outcomes) / length(treated_outcomes)
control_sepsis_rate <- sum(control_outcomes) / length(control_outcomes)
matched_risk_diff <- treated_sepsis_rate - control_sepsis_rate

# Calculate odds ratio for McNemar's test
# OR = (discordant pairs in favor of treatment) / (discordant pairs against treatment)
if(discordant_treated_sepsis > 0) {
  mcnemar_or <- discordant_treated_sepsis / discordant_control_sepsis
} else {
  mcnemar_or <- Inf
}

# Calculate confidence interval for OR
if(discordant_control_sepsis > 0 && discordant_treated_sepsis > 0) {
  log_or <- log(mcnemar_or)
  se_log_or <- sqrt(1/discordant_control_sepsis + 1/discordant_treated_sepsis)
...
clogit_model <- clogit(sepsis ~ livertxp + strata(subclass), data = matched_data)
...
results_table <- data.frame(
  ...
  Odds_Ratio = c(
    ...
    sprintf("%.2f", exp(coef(clogit_model)["livertxp"]))
  ),
  P_value = c(
    sprintf("%.4f", mcnemar_result$p.value),
    sprintf("%.4f", summary(clogit_model)$coefficients["livertxp", "Pr(>|z|)"])
  ),
  CI_95 = c(
    ifelse(is.finite(mcnemar_or) && !is.na(ci_lower), 
           sprintf("%.2f-%.2f", ci_lower, ci_upper), "Not calculated"),
    sprintf("%.2f-%.2f", exp(confint(clogit_model)["livertxp", 1]), exp(confint(clogit_model)["livertxp", 2]))
  ),
  Significant = c(
    ifelse(mcnemar_result$p.value < 0.05, "Yes", "No"),
    ifelse(summary(clogit_model)$coefficients["livertxp", "Pr(>|z|)"] < 0.05, "Yes", "No")
  )
)
...
cat(sprintf("Discordant pairs: %d (analysis based on these pairs)\n", 
            discordant_control_sepsis + discordant_treated_sepsis))

cat(sprintf("\nPost-Matching Effect Sizes:\n"))
cat(sprintf("Risk Difference: %.3f (%.1f percentage points)\n", matched_risk_diff, matched_risk_diff * 100))
cat(sprintf("Control group sepsis rate: %.1f%% (%d/%d)\n", 
            control_sepsis_rate * 100, sum(control_outcomes), length(control_outcomes)))
cat(sprintf("Treated group sepsis rate: %.1f%% (%d/%d)\n", 
            treated_sepsis_rate * 100, sum(treated_outcomes), length(treated_outcomes)))
...
cat(sprintf("Matching efficiency: %d/%d (100.0%%)\n", 
            sum(matched_data$livertxp == 1), sum(my_data$livertxp == 1)))

# Final conclusion
if(mcnemar_result$p.value < 0.05) {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows statistically significant difference in sepsis risk between matched pairs ***\n")
} else {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows no statistically significant difference in sepsis risk between matched pairs ***\n")
}
