# Simplified PSM Analysis for Liver Transplant - Binary Outcome (Renal Failure)
library(MatchIt)
library(tableone)
library(cobalt)
library(ggplot2)
...
print(table(my_data$renalfailure, my_data$livertxp))
...
pre_outcome_table <- table(my_data$renalfailure, my_data$livertxp, 
                           dnn = c("Renal Failure", "Liver Transplant"))
...
love_plot <- love.plot(psmatch,
                       ...
                       position = "bottom",
                       var.names = var.names,
                       title = "Balance Before and After Matching: Liver Transplant Analysis") +
  theme_bw() +
  theme(
...
treated_outcomes <- matched_pairs$renalfailure[matched_pairs$livertxp == 1]
control_outcomes <- matched_pairs$renalfailure[matched_pairs$livertxp == 0]
...
cat(sprintf("Both had no renal failure: %d pairs (%.1f%%)\n", 
            concordant_both_survive, (concordant_both_survive/total_pairs)*100))
cat(sprintf("Both had renal failure: %d pairs (%.1f%%)\n", 
            concordant_both_fail, (concordant_both_fail/total_pairs)*100))
cat(sprintf("Control had renal failure, Treated did not: %d pairs (%.1f%%)\n", 
            discordant_control_fail, (discordant_control_fail/total_pairs)*100))
cat(sprintf("Control had no renal failure, Treated did: %d pairs (%.1f%%)\n", 
...
clogit_model <- clogit(renalfailure ~ livertxp + strata(subclass), data = matched_data)
...
cat(sprintf("Control group renal failure rate: %.1f%% (%d/%d)\n", 
            control_failure_rate * 100, sum(control_outcomes), length(control_outcomes)))
cat(sprintf("Treated group renal failure rate: %.1f%% (%d/%d)\n", 
