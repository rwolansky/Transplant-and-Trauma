# Simplified PSM Analysis for Liver Transplant - Binary Outcome (Liver Failure)
library(MatchIt)
library(tableone)
library(cobalt)
library(ggplot2)
...
print(table(my_data$liverfailure, my_data$livertxp))
...
pre_outcome_table <- table(my_data$liverfailure, my_data$livertxp, 
                           dnn = c("Liver Failure", "Liver Transplant"))
...
treated_outcomes <- matched_pairs$liverfailure[matched_pairs$livertxp == 1]
control_outcomes <- matched_pairs$liverfailure[matched_pairs$livertxp == 0]
...
clogit_model <- clogit(liverfailure ~ livertxp + strata(subclass), data = matched_data)
...
if(mcnemar_result$p.value < 0.05) {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows statistically significant difference in liver failure risk between matched pairs ***\n")
} else {
  cat("\n*** FINAL CONCLUSION: McNemar's test shows no statistically significant difference in liver failure risk between matched pairs ***\n")
}
